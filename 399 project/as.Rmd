...
abstract: |
  This study seeks to establish a casual relationship between gun ownership and gun crime, in a manner that can be extrapolated to overall levels of murder. The study found a strong correlation
...
From here,the model will be refined by adding further independent variables in order to control for socio-economic factors. The first  additional variable included was the poverty rate, which in itself is a strong predictor of crime. It is worth noting that from here the intercept coffecient ceases to be significant; also that controlling for poverty causes reduction of 0.037 murders per 1% increase in households owning guns. The overall relationship is given by the formula below:
...
```
\
Educational attainment also serves as a strong predictor of overall crime, and captures a number of factors we should wish to control for. As expected, education has a negative overall impact on murder rates, and the inclution of this control sees the impact of gun ownership decline slightly over the previous model. As usual, the relationship given by this model is given in the formula below:
...
The final control variable used was the per capita divorce rate for each state. This captures a number of key factors should be controlled for, such as the stability of home life during childhood. It is worth noting that within the American political arena, decline in family values is often blamed for gun violence. This is only a half truth, as the overall divorce rate has been steadily dropping for several decades within the United States. However, the final model did find divorce rates to be a strong predicter of homicide rates. 

```{r}
usmodelfinal <- lm(agemurder~gunrate+poverty+borderchainav+divorce+educ, data=crimestat3)
susmodelf <- summary(usmodelfinal)
tablf <- data.frame(xtable(susmodelf))
names(tablf) <- c("Estimate", "Std. Error", "t", "p-Value")
kable(tablf, digits=3, align='c')
print('R²=0.7369')
```

Additional inference can be drawn from lumping states with similar levels of gun ownership together and finding factors that lead to differences between these groups. With this procedure, 3 different groups have been created. Alabama, Arkansas, Idaho, Montana, and West Virginia, South Dakota all have rates of gun ownership in the mid 50s yet considerable variance is seen in crime rates between our states. The second group is comprised of Hawaii and the District of Columbia. Both of these states have next to no gun ownership, yet have a massive disparity in
...
Overall, this paper finds gun ownership per capita to be a significant factor in explaining the large variation of overall level of homicide between US states. Within the final model, a 1% increase in households owning firearms causes an increase in the per capita homicide rate by
...
fit1 <- lm(agemurder~gunrate, data=crimestat3)
fit2 <- lm(agemurder~gunrate+poverty, data=crimestat3)
fit3 <- lm(agemurder~gunrate+poverty+borderchainav+divorce+educ, data=crimestat3)
sjt.lm(fit1, fit2)
```
